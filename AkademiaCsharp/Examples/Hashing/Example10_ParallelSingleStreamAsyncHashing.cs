...
{
    public class Example10_ParallelSingleStreamAsyncHashing : IExample
    {
        ...
        public Example10_ParallelSingleStreamAsyncHashing(IAsyncHasher hasher, int streamLength, int seed, int count)
        {
            _hasher = hasher;
        ...
        public async Task<bool> InvokeAsync(ITimeMeasurer timeMeasurer, CancellationToken token)
        {
            ...
            {
                hashTasks.Add(Hash(_hasher, randomStream, token));
            }
        ...
        private Task<byte[]> Hash(IAsyncHasher hasher, RandomStream stream, CancellationToken token)
        {
            return hasher.HashAsync(stream, token);
        }
    }
}
